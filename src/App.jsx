...
function App() {
  ...
  useEffect(() => {
    ...
    async function fetchMovies() {
      try {
        const response = await fetch(`https://omdbapi.com/?apikey=${API_KEY}&s=${searchTerm}`);
    ...
    <div className="bg-slate-900 text-slate-200 min-h-screen p-4 sm:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="flex gap-2 mb-8 max-w-xl mx-auto">
          ...
          />
          <button type="submit" className="p-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">Search</button>
        </form>

        <div className='content'>
          {isLoading && <h3 className="text-center text-slate-400">Loading...</h3>}
          {error && <h3 className="text-center text-red-500">Error: {error}</h3>}
          
          {!isLoading && !error && movies.length > 0 && (
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6'>
              {movies.map(movie => (
                <div key={movie.imdbID} className='bg-slate-800 rounded-lg shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300'>
                  <img src={movie.Poster !== 'N/A' ? movie.Poster : 'https://via.placeholder.com/300x444.png?text=No+Image'} alt={movie.Title} className="w-full h-96 object-cover" />
                  <div className="p-4">
          ...
          {!isLoading && !error && movies.length === 0 && searchTerm && (
             <h3 className="text-center text-slate-400">No movies found for "{searchTerm}". Please try another search.</h3>
          )}
        </div>
      </div>
    </div>
